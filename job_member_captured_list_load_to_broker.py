...
df = dymc_frame.toDF().groupBy("broker_code","broker","branch_name").count()

df_sel = df.select( "broker_code"
                    , col("broker").alias("broker_name")
                    , col("branch_name").alias("broker_branch")
                    , hex(sha1(concat(encode(df.broker_code.cast("string"),"UTF-8"), encode(df.broker.cast("string"),"UTF-8"), encode(df.branch_name.cast("string"),"UTF-8")))).alias('hash_key')
                    , hex(sha1(concat(encode(df.broker_code.cast("string"),"UTF-8"), encode(df.broker.cast("string"),"UTF-8"), encode(df.branch_name.cast("string"),"UTF-8")))).alias('hash_diff')
