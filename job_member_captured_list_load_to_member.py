...
df = dymc_frame.toDF().groupBy("first_name","last_name","id_number","gender","date_of_birth").count()

df_sel = df.select( upper(col("first_name")).alias("first_name")
                    , upper(col("last_name")).alias("last_name")
                    , "id_number"
                    , "gender"
                    , to_date(from_unixtime(unix_timestamp("date_of_birth", "yyyy-MM-dd"), "dd-MM-yyyy"), "dd-MM-yyyy").alias("birth_date")
                    , lit('').alias("title")
