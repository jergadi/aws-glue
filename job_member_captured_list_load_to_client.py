...
df = dymc_frame.toDF().groupBy("client").count()

df_sel = df.select( col("client").alias("client_name")
                    , hex(sha1(concat(encode(df.client.cast("string"),"UTF-8")))).alias('hash_key')
                    , hex(sha1(concat(encode(df.client.cast("string"),"UTF-8")))).alias('hash_diff')
...
datasink5 = glue_context.write_dynamic_frame.from_catalog(frame = dynamic_frame_write, database = "kgalife", table_name = "postgres_staging_client", transformation_ctx = "datasink5")
