...
df_sel = df.select( upper(col("client")).alias("client_name")
                    , hex(sha1(encode(df.client.cast("string"),"UTF-8"))).alias('hash_key')
                    , hex(sha1(encode(df.client.cast("string"),"UTF-8"))).alias('hash_diff')
